            ...
            if (s + table_data[i][j].colspan) == old[k].colspan:
                if s == 0:
                    ...
                    while k < len(old):
                        ...
                        # Проверяем, что родительская ячейка закончилась
                        if old[k].rowspan > 1:
                            old[k].classCell = CellType.ClassCell.CELL_NOT_DEFINE
                            k += 1
                            continue
...
def get_data(table: Table) -> Table:
    ...
    old = [cell for cell in table_data[start] if cell.classCell!=CellType.ClassCell.CELL_SIDEBAR]
    ...
    for i in range(start + 1, len(table_data)):
        ...
        while k < len(old):
            ...
            if table_data[i][j].classCell == CellType.ClassCell.CELL_SIDEBAR:
                j +=1
                continue
